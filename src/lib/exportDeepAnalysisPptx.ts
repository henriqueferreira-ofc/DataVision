...
export function exportDeepAnalysisPptx(analysis: AnalysisData, language: string) {
  ...
  const footer = (slide: any) => {
    slide.addText(
      [
        { text: "DataVision", options: { bold: true, color: C.gray400, fontSize: 8 } },
  ...
  cover.addText("D A T A V I S I O N", { x: 1, y: 1.2, w: 10, fontSize: 16, color: C.blue, bold: true });
  ...
  closing.addText("D A T A V I S I O N", { x: 1, y: 2.0, w: 10, fontSize: 16, color: C.blue, bold: true });
  closing.addText(pt ? "Obrigado" : "Thank You", { x: 1, y: 3.0, w: 10, fontSize: 40, color: C.white, bold: true });
  closing.addText(pt ? "Relatório gerado automaticamente pela plataforma DataVision" : "Report automatically generated by DataVision platform", {
